const fs = require('fs');
let content = fs.readFileSync('src/lib/googleWorkspace.ts', 'utf-8');

const regex = /const metadata = \{[\s\S]*?throw new Error\("Failed to get webViewLink( from Google Drive)?"\);\s*\}/;

const newUpload = `    const metadata = {
      name: fileName,
      mimeType: mimeType,
    };
...
fs.writeFileSync('src/lib/googleWorkspace.ts', content);
