...
async function startServer() {
  const app = express();
  // Convert PORT to a number — process.env.PORT is a string, but net.Server.listen() needs a number
  ...
  if (process.env.NODE_ENV !== 'production') {
    ...
    // Without this, esbuild would emit require('vite') at the top of dist/server.cjs
    // which crashes the container because vite is not installed with --omit=dev.
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-ignore — vite types are only available as a devDependency; the import
    // is guarded by NODE_ENV !== 'production' so it is never called at runtime in Docker.
    const loadVite = new Function('specifier', 'return import(specifier)') as
      (specifier: string) => Promise<typeof import('vite')>;
    const { createServer: createViteServerDyn } = await loadVite('vite');
